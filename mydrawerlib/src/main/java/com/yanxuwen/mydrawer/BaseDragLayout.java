...
public abstract class BaseDragLayout extends ViewGroup {
    ...
    public BaseDragLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DragLayout);
            ...
            public void onViewReleased(View releasedChild, float xvel, float yvel) {
                ...
                switch (getMode()) {
                    ...
                    case MODE_DRAG_LEFT:
                        ...
                        mDragHelper.settleCapturedViewAt(xvel > 0 || (xvel == 0 && offset_left > 0.5f * maxShowScale) ? openLeft : -childWidth, releasedChild.getTop());
                        invalidate();
                        break;
                    case MODE_DRAG_RIGHT:
                        ...
                        mDragHelper.settleCapturedViewAt(xvel < 0 || (xvel == 0 && offset_right > 0.5f * maxShowScale) ? openRight : getWidth(), releasedChild.getTop());
                        invalidate();
                        break;
                    case MODE_DRAG_TOP:
                        //0~1f,关到开
                        ...
                        mDragHelper.settleCapturedViewAt(releasedChild.getLeft(), yvel > 0 || (yvel == 0 && offset_top > 0.5f * maxShowScale) ? openTop : -childHeight);
                        invalidate();
                        break;
                ...
                if (childView == mDescView) {
                    ...
                    switch (getMode()) {
                        case MODE_DRAG_LEFT:
                            ...
                            childView.layout(childLeft, lp.topMargin, childLeft + menuWidth, lp.topMargin + menuHeight);
                            break;
                        case MODE_DRAG_RIGHT:
                        ...
                        case MODE_DRAG_TOP:
                            ...
                            childView.layout(lp.leftMargin, childTop, lp.leftMargin + menuWidth, childTop + menuHeight);
                            break;
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (mRecyclerView != null) {
                switch (getMode()) {
                    ...
                    case MODE_DRAG_LEFT:
                        if ((mRecyclerView != null && isRecyclerViewRight) && (downX - ev.getX()) > 1)
                            return true;
                    case MODE_DRAG_RIGHT:
                        if ((mRecyclerView != null && isRecyclerViewLeft) && (ev.getX() - downX) > 1)
                            return true;
                    case MODE_DRAG_TOP:
                        if ((mRecyclerView != null && isRecyclerViewBottom) && (downY - ev.getY()) > 1)
                            return true;
                    case MODE_DRAG_BOTTOM:
                        if ((mRecyclerView != null && isRecyclerViewTop) && (ev.getY() - downY) > 1)
                            return true;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isOutside && isopen && event.getAction() == MotionEvent.ACTION_UP) {
            if (Math.abs(event.getX() - downX) < 50 && Math.abs(event.getY() - downY) < 50) {
                if (!(event.getX() >= getContentView().getX() && event.getX() <= getContentView().getX() + getContentView().getWidth() && event.getY() >= getContentView().getY() && event.getY() <= getContentView().getY() + getContentView().getHeight())) {
                    close();
                    return true;
                }
            }
        ...
        return mLeftMenuOnScreen != 0 || (isReturn);
    }
    ...
    private boolean isRecyclerViewHorizontal() {
        ...
        if (mLayoutManager instanceof LinearLayoutManager) {
            return ((LinearLayoutManager) mLayoutManager).getOrientation() == LinearLayoutManager.HORIZONTAL;
        }
        return false;
    }
}
